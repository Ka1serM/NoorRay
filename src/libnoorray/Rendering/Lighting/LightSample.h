...
struct LightSample
{
    ...
    // Index into GpuSceneData::directLightCandidates. This keeps the sampled
    // light identity attached to the sample so the same selection PDF can be
    // used by direct lighting and emissive-hit MIS.
    uint32_t candidateIndex{~uint32_t{0}};
