...
void launchGenerate(const KernelParams& params, cudaStream_t stream);
...
void launchGenerate(const KernelParams& params, const cudaStream_t stream)
{
    constexpr uint32_t blockSize = 256;
    const uint32_t count = params.frame.width * params.frame.height;
    generateKernel<<<(count + blockSize - 1) / blockSize, blockSize, 0, stream>>>(params);
}
