 ...
 *
 * Fractal-noise node port. The octave loop is structurally the same as
 * Cycles' fractal_noise.h; the Perlin primitive follows MaterialX noise.mdl,
 * whose standard fractal3d node is defined in terms of OSL-compatible noise.
 */
...
{
    const int x = static_cast<int>(floorf(p.x)), y = static_cast<int>(floorf(p.y));
    const int z = static_cast<int>(floorf(p.z));
    const float fx = p.x - floorf(p.x), fy = p.y - floorf(p.y), fz = p.z - floorf(p.z);
...
NR_GPU inline float svmPerlin2d(const glm::vec2 p)
{
    ...
    const float fx = p.x - floorf(p.x), fy = p.y - floorf(p.y);
