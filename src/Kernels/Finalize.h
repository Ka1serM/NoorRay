...
void launchFinalize(const KernelParams& params, cudaStream_t stream);
...
NR_GPU_KERNEL void finalizeKernel(const KernelParams params)
{
    ...
    radiance = lerp3(glm::vec3(previous.x, previous.y, previous.z), radiance, weight);
    ...
    if (params.frame.sampleIndex == 0)
    {
        ...
        const glm::vec3 albedo = clamp3(primary.primaryAlbedo, 0.0f, 1.0f);
    ...
    surf2Dwrite(packHalf4(glm::vec3(mean, m2, count), 0.0f), params.output.material, x * sizeof(ushort4), y);
}

void launchFinalize(const KernelParams& params, const cudaStream_t stream)
{
    constexpr uint32_t blockSize = 256;
    const uint32_t count = params.frame.width * params.frame.height;
    finalizeKernel<<<(count + blockSize - 1) / blockSize, blockSize, 0, stream>>>(params);
}
