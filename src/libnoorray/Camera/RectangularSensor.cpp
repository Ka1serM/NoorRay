...
bool Sensor::loadImageSensorDimensions() {
    ...
    catch(const std::exception& e) { std::snprintf(imageSensorLoadStatus,sizeof(imageSensorLoadStatus),"%s",e.what()); NR_LOG_ERROR("Sensor: " << imageSensorLoadStatus); return false; }
}
