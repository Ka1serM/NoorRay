...
public:
    ...
    NR_CPU_GPU const std::vector<Vertex>& getVertices() const { return vertices; }
    NR_CPU_GPU const std::vector<uint32_t>& getIndices() const { return indices; }
    NR_CPU_GPU const std::vector<Face>& getFaces() const { return faces; }
    NR_CPU_GPU const std::vector<uint32_t>& getMaterialIds() const {
        return materialIds;
    }
    // Backend-neutral geometry mirror. Vulkan keeps host vectors
    // for the existing path, while Vulkan uploads these host-owned records
    // without requiring a separate device context or interop allocation.
    const std::vector<Vertex>& getHostVertices() const { return hostVertices; }
    const std::vector<uint32_t>& getHostIndices() const { return hostIndices; }
    const std::vector<Face>& getHostFaces() const { return hostFaces; }
