...
// Returns a canonical default MaterialX surface document: a neutral
// open_pbr_surface wrapped in a surfacematerial. This is what a material is
// lowered to when it has no authored document at all, and what a graph is
// fallen back to when it no longer compiles (for example after deleting an
// important node).
MaterialX::DocumentPtr defaultMaterial();

// Lowers one authoring struct -- the simple record an importer fills in --
// into a canonical MaterialX surface graph (an open_pbr_surface wrapped in a
// surfacematerial). Importers convert their materials through this function
