...
void CpuRaytracer::render(const vk::CommandBuffer& commandBuffer, const PushConstants& pushConstants) {
    ...
    auto render_tile = [this, &pushConstants](uint32_t start_y, uint32_t end_y) {
    ...
    for (uint32_t i = 0; i < num_threads; ++i) {
        ...
        threads.emplace_back(render_tile, start_y, end_y);
    }

    for (auto& t : threads)
        t.join(); // This blocks the main render thread
...
void CpuRaytracer::traceRayEXT_CPU(Payload& payload, float tMin, float tMax) {
    ...
    for (const auto* instance : threadSafeInstances) {
        ...
        HitInfo localHit;
        if (asset->blasCpu.intersect(local_origin, local_dir, localHit, tMin, tMax_local)) {
...
void CpuRaytracer::closesthit(const HitInfo& hit, Payload& payload) {
    ...
    const auto& meshIndices = meshAsset->indices;
    ...
    const auto& meshVertices = meshAsset->vertices;
    ...
    const Face& face = meshAsset->faces[hit.primitiveIndex];
    Material material = meshAsset->materials[face.materialIndex];
