...
public:
    ...
    // Convenience overload for standalone callers that do not provide a
    // ray interval. The path tracer uses the bounded form above.
    NR_CPU_GPU LightHit intersect(
        const Ray& ray, const SampledWavelengths& wl,
        const float* spectrumScale, const float* spectrumCoeffs,
        const float* d65) const
    {
        return intersect(ray, 0.0f, Ray::InfiniteDistance, wl,
            spectrumScale, spectrumCoeffs, d65);
    }
