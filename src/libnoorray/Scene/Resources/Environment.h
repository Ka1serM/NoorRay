...
public:
    ...
    NR_GPU float pdf(const glm::vec3 direction) const
    {
        ...
        // computeCdf() stores the final solid-angle PDF in alpha. Besides
        // being cheaper than reconstructing two CDF differences and a pixel
        // solid angle, this guarantees that sampling and MIS use the exact
        // same discretized distribution.
        return fmaxf(cdfTexel(x, y).w, 0.0f);
    }
    ...
    NR_GPU EnvironmentSample sampleDirection(Rng& rng) const
    {
        ...
        // The CDF alpha channel already stores the exact solid-angle PDF of
        // the selected texel. Reuse it directly instead of remapping the
        // sampled direction and issuing a second CDF texture lookup.
        sample.pdf = fmaxf(cdfTexel(x, y).w, 0.0f);
        return sample;
    }
