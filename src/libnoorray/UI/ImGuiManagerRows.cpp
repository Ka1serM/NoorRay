...
void ImGuiManager::colorEdit3Row(const char* label, const glm::vec3 value,
                                 const std::function<void(glm::vec3)>& setter) {
    tableRowLabel(label);
    if (glm::vec3 temp = value;
        ImGui::ColorEdit3((std::string("##") + label).c_str(), glm::value_ptr(temp)))
        setter(temp);
}
...
void ImGuiManager::colorEdit4Row(const char* label, const glm::vec4 value,
    ...
    tableRowLabel(label);
    if (glm::vec4 temp = value;
        ImGui::ColorEdit4((std::string("##") + label).c_str(), glm::value_ptr(temp)))
        setter(temp);
}
