...
float decodeHalf(const half value)
{
...
const std::vector<half>& Texture::getHalfPixels() const
{
    return halfPixels ? *halfPixels : emptyStorage<half>();
}
    ...
    , encoding(encoding)
{
    ...
    if (encoding == TextureEncoding::Float32) {
        ...
        auto storage = std::make_shared<std::vector<half>>(valueCount);
        std::memcpy(
            storage->data(), data, storage->size() * sizeof(half));
...
Texture::Texture(std::string textureName, std::vector<half>&& data,
    ...
    const TextureEncoding textureEncoding)
    : Texture(std::move(textureName),
        std::make_shared<const std::vector<half>>(std::move(data)),
        textureWidth, textureHeight, textureEncoding)
{
}
...
Texture::Texture(std::string textureName,
    std::shared_ptr<const std::vector<half>> data,
    const int textureWidth, const int textureHeight,
    const TextureEncoding textureEncoding)
    : name(std::move(textureName))
    , width(textureWidth)
