...
public:
    ...
    // Evaluates a sampled light, including the receiving-surface cosine.
    // Keeping this beside evaluate() makes direct-light estimators consumers
    // of the BSDF rather than of its shading-frame implementation details.
    NR_CPU_GPU SampledSpectrum evaluateDirect(
        const glm::vec3& light, const SampledSpectrum& radiance) const
    {
        const float cosine = fmaxf(glm::dot(shadingNormal, light), 0.0f);
        return cosine > 0.0f ? evaluate(light) * radiance * cosine
...
public:
    ...
    // Public (rather than accessor methods) since callers such as the shade kernel
