...
HostParameters parametersFromScene(const Scene& scene)
{
    ...
    const auto& gaussians = scene.getGaussianAsset(instances.front()->getGaussianAssetIndex()).getGaussians();
    HostParameters result{std::vector<float>(gaussians.size() * 3), std::vector<float>(gaussians.size() * 3),
        std::vector<float>(gaussians.size() * 4), std::vector<float>(gaussians.size()),
        std::vector<float>(gaussians.size() * 3)};
