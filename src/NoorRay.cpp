...
void NoorRay::run() {
    ...
    while (isRunning) {
        ...
        try {
            ...
            imGuiManager.Draw(commandBuffer, renderer.getCurrentSwapchainImageIndex());
...
void NoorRay::setupScene() {
    ...
    auto cam = std::make_unique<PerspectiveCamera>(scene, "Camera", Transform{vec3(0,0,-5.0f), vec3(0), vec3(1)}, aspectRatio, 36.0f, 24.0f, 45.0f, 1.8f, 5.0f, 2.0f);
