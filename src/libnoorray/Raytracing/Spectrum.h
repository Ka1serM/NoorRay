// Adapted from pbrt-v4 (Apache-2.0)
// Original: Matt Pharr, Wenzel Jakob, Greg Humphreys
// https://github.com/mmp/pbrt-v4/blob/master/src/pbrt/util/spectrum.h
// https://github.com/mmp/pbrt-v4/blob/master/src/pbrt/util/color.h
// https://github.com/mmp/pbrt-v4/blob/master/src/pbrt/util/sampling.h
#pragma once
...
static constexpr float NrLambdaMin      = 360.f;
static constexpr float NrLambdaMax      = 830.f;
static constexpr int   NrSpectrumSamples = 4;
static constexpr float NrCIE_Y_integral  = 106.856895f;
static constexpr int   NrD65Samples      = 107;
// PBRT normalizes CIE D65 so its integral against CIE Y equals CIE_Y_integral.
static constexpr float NrD65Normalization = 0.01011224743f;

// CIE standard illuminant D65, 300--830 nm in 5 nm increments. Values are
// mechanically copied from pbrt-v4's CIE_Illum_D6500 table.
static constexpr float NrD65[NrD65Samples] = {
    0.034100f, 1.664300f, 3.294500f, 11.765200f, 20.236000f, 28.644699f,
    37.053501f, 38.501099f, 39.948799f, 42.430199f, 44.911701f, 45.775002f,
    46.638302f, 49.363701f, 52.089100f, 51.032299f, 49.975498f, 52.311798f,
    54.648201f, 68.701500f, 82.754898f, 87.120399f, 91.486000f, 92.458900f,
    93.431801f, 90.056999f, 86.682297f, 95.773598f, 104.864998f, 110.935997f,
    117.008003f, 117.410004f, 117.811996f, 116.335999f, 114.861000f, 115.391998f,
    115.922997f, 112.366997f, 108.810997f, 109.082001f, 109.353996f, 108.578003f,
    107.802002f, 106.295998f, 104.790001f, 106.238998f, 107.689003f, 106.046997f,
    104.404999f, 104.224998f, 104.045998f, 102.023003f, 100.000000f, 98.167099f,
    96.334198f, 96.061096f, 95.788002f, 92.236801f, 88.685600f, 89.345901f,
    90.006203f, 89.802597f, 89.599098f, 88.648903f, 87.698700f, 85.493599f,
    83.288597f, 83.493896f, 83.699203f, 81.862999f, 80.026802f, 80.120697f,
    80.214600f, 81.246201f, 82.277802f, 80.280998f, 78.284203f, 74.002701f,
    69.721298f, 70.665199f, 71.609100f, 72.978996f, 74.348999f, 67.976501f,
    61.604000f, 65.744797f, 69.885597f, 72.486298f, 75.086998f, 69.339798f,
    63.592701f, 55.005402f, 46.418201f, 56.611801f, 66.805397f, 65.094101f,
    63.382801f, 63.843399f, 64.304001f, 61.877899f, 59.451900f, 55.705399f,
    51.959000f, 54.699799f, 57.440601f, 58.876499f, 60.312500f,
};

// ---------------------------------------------------------------------------
// SampledSpectrum — NrSpectrumSamples scalar values at sampled wavelengths
// Adapted from pbrt-v4 SampledSpectrum
// ---------------------------------------------------------------------------
class SampledSpectrum
{
public:
    NR_CPU_GPU SampledSpectrum() {
        for (int i = 0; i < NrSpectrumSamples; ++i) values[i] = 0.f;
    }
    NR_CPU_GPU explicit SampledSpectrum(float c) {
        for (int i = 0; i < NrSpectrumSamples; ++i) values[i] = c;
    }

    NR_CPU_GPU float  operator[](int i) const { return values[i]; }
    NR_CPU_GPU float& operator[](int i)       { return values[i]; }

    NR_CPU_GPU explicit operator bool() const {
        for (int i = 0; i < NrSpectrumSamples; ++i)
            if (values[i] != 0.f) return true;
        return false;
    }

    NR_CPU_GPU float average() const {
        float s = 0.f;
        for (int i = 0; i < NrSpectrumSamples; ++i) s += values[i];
        return s / NrSpectrumSamples;
    }
    NR_CPU_GPU float maxComponent() const {
        float m = values[0];
        for (int i = 1; i < NrSpectrumSamples; ++i) m = fmaxf(m, values[i]);
        return m;
    }

    NR_CPU_GPU SampledSpectrum& operator+=(const SampledSpectrum& s) {
        for (int i = 0; i < NrSpectrumSamples; ++i) values[i] += s.values[i];
        return *this;
    }
    NR_CPU_GPU SampledSpectrum operator+(const SampledSpectrum& s) const {
        SampledSpectrum r = *this; return r += s;
    }
    NR_CPU_GPU SampledSpectrum& operator-=(const SampledSpectrum& s) {
        for (int i = 0; i < NrSpectrumSamples; ++i) values[i] -= s.values[i];
        return *this;
    ...
    NR_CPU_GPU SampledSpectrum& operator*=(float a) {
        for (int i = 0; i < NrSpectrumSamples; ++i) values[i] *= a;
        return *this;
    }
    NR_CPU_GPU SampledSpectrum operator*(float a) const {
        SampledSpectrum r = *this; return r *= a;
    }
    NR_CPU_GPU SampledSpectrum& operator/=(float a) {
        for (int i = 0; i < NrSpectrumSamples; ++i) values[i] /= a;
        return *this;
    ...
    float values[NrSpectrumSamples];
};

NR_CPU_GPU inline SampledSpectrum operator*(float a, const SampledSpectrum& s) { return s * a; }

// SafeDiv: returns 0 where denominator is 0 (from pbrt-v4)
NR_CPU_GPU inline SampledSpectrum safeDiv(const SampledSpectrum& a, const SampledSpectrum& b)
{
    SampledSpectrum r;
    for (int i = 0; i < NrSpectrumSamples; ++i)
        r.values[i] = (b.values[i] != 0.f) ? a.values[i] / b.values[i] : 0.f;
    return r;
}

NR_CPU_GPU inline SampledSpectrum clampZero(SampledSpectrum s)
{
    for (int i = 0; i < NrSpectrumSamples; ++i) s.values[i] = fmaxf(0.f, s.values[i]);
    return s;
}

// ---------------------------------------------------------------------------
// SampledWavelengths — NrSpectrumSamples wavelengths + their sampling PDFs
// Adapted from pbrt-v4 SampledWavelengths
// ---------------------------------------------------------------------------
class SampledWavelengths
{
public:
    NR_CPU_GPU float  operator[](int i) const { return lambda[i]; }

    NR_CPU_GPU SampledSpectrum PDF() const {
        SampledSpectrum s;
        for (int i = 0; i < NrSpectrumSamples; ++i) s.values[i] = pdf[i];
        return s;
    }

    // Uniform stratified sampling across [LambdaMin, LambdaMax] with wraparound.
    // u in [0,1). From pbrt-v4 SampledWavelengths::SampleUniform.
    NR_CPU_GPU static SampledWavelengths sampleUniform(float u)
    {
        SampledWavelengths wl;
        wl.lambda[0] = NrLambdaMin + u * (NrLambdaMax - NrLambdaMin);
        const float delta = (NrLambdaMax - NrLambdaMin) / NrSpectrumSamples;
        for (int i = 1; i < NrSpectrumSamples; ++i) {
            wl.lambda[i] = wl.lambda[i - 1] + delta;
            if (wl.lambda[i] > NrLambdaMax)
                wl.lambda[i] = NrLambdaMin + (wl.lambda[i] - NrLambdaMax);
        }
        const float p = 1.f / (NrLambdaMax - NrLambdaMin);
        for (int i = 0; i < NrSpectrumSamples; ++i) wl.pdf[i] = p;
        return wl;
    }

    // Stratified sampling biased toward the photopic-sensitivity peak (~555 nm).
    // From pbrt-v4 SampledWavelengths::SampleVisible.
    NR_CPU_GPU static SampledWavelengths sampleVisible(float u)
    {
        SampledWavelengths wl;
        for (int i = 0; i < NrSpectrumSamples; ++i) {
            float up = u + float(i) / NrSpectrumSamples;
            if (up > 1.f) up -= 1.f;
            wl.lambda[i] = sampleVisibleWavelengths(up);
            wl.pdf[i]    = visibleWavelengthsPDF(wl.lambda[i]);
        }
        return wl;
    }
    ...
    NR_CPU_GPU void terminateSecondary()
    {
        ...
        for (int i = 1; i < NrSpectrumSamples; ++i) pdf[i] = 0.f;
        pdf[0] /= NrSpectrumSamples;
    }
    NR_CPU_GPU bool secondaryTerminated() const
    {
        for (int i = 1; i < NrSpectrumSamples; ++i)
    ...
    float pdf[NrSpectrumSamples];

private:
    // From pbrt-v4 util/sampling.h
    NR_CPU_GPU static float sampleVisibleWavelengths(float u) {
        return 538.f - 138.888889f * atanhf(0.85691062f - 1.82750197f * u);
    }
    NR_CPU_GPU static float visibleWavelengthsPDF(float lambda) {
        if (lambda < 360.f || lambda > 830.f) return 0.f;
        const float x = 0.0072f * (lambda - 538.f);
        const float ch = coshf(x);
        return 0.0039398042f / (ch * ch);
    }
};

// ---------------------------------------------------------------------------
// CIE 1931 2° observer CMF lookup
// Table: 471 entries at 1 nm steps, offset = round(lambda) - 360.
// Adapted from pbrt-v4 DenselySampledSpectrum::Sample (lambda_min=360).
// ---------------------------------------------------------------------------
NR_CPU_GPU inline float sampleCIETable(const float* table, float lambda)
{
    const int offset = static_cast<int>(lambda + 0.5f) - 360;
    if (offset < 0 || offset >= 471) return 0.f;
    return table[offset];
}

// Spectral radiance → XYZ via Monte Carlo estimator.
// Equivalent to pbrt-v4 SampledSpectrum::ToXYZ.
// cieX/Y/Z: device (or host) pointers to 471-float CIE CMF tables.
NR_CPU_GPU inline glm::vec3 spectrumToXYZ(
    ...
    const float*              cieZ)
{
    ...
    for (int i = 0; i < NrSpectrumSamples; ++i) {
        X.values[i] = sampleCIETable(cieX, wl.lambda[i]);
        Y.values[i] = sampleCIETable(cieY, wl.lambda[i]);
        Z.values[i] = sampleCIETable(cieZ, wl.lambda[i]);
    }
    const SampledSpectrum pdf = wl.PDF();
    return glm::vec3(
        safeDiv(X * L, pdf).average(),
        safeDiv(Y * L, pdf).average(),
        safeDiv(Z * L, pdf).average()
    ) / NrCIE_Y_integral;
}
    ...
    const float*              cieY)
{
    ...
    for (int i = 0; i < NrSpectrumSamples; ++i)
        Y.values[i] = sampleCIETable(cieY, wl.lambda[i]);
    const SampledSpectrum pdf = wl.PDF();
    return safeDiv(Y * L, pdf).average() / NrCIE_Y_integral;
}

// XYZ → linear sRGB (D65 whitepoint, ITU-R BT.709 primaries).
// Matrix from IEC 61966-2-1.
NR_CPU_GPU inline glm::vec3 xyzToLinearSRGB(const glm::vec3& xyz)
{
    return glm::vec3(
...
// Linear → sRGB-encoded (IEC 61966-2-1 transfer function).
// Minimax polynomial approximation from pbrt-v4 util/color.h (enoki's color.h).
NR_CPU_GPU inline float linearToSRGB(float value)
{
    if (value <= 0.0031308f)
        return 12.92f * value;
    const float sq = sqrtf(fmaxf(0.f, value));
    // Horner evaluation of numerator and denominator polynomials (from pbrt-v4)
    const float p =
        -0.0016829072605308378f +
        sq * ( 0.03453868659826638f +
        sq * ( 0.7642611304733891f +
        sq * ( 2.0041169284241644f +
        sq * ( 0.7551545191665577f +
        sq * -0.016202083165206348f))));
    const float q =
         4.178892964897981e-7f +
        sq * (-0.00004375359692957097f +
        sq * ( 0.03467195408529984f +
        sq * ( 0.6085338522168684f +
        sq * ( 1.8970238036421054f +
        sq *   1.f))));
    return (p / q) * value;
}

// ---------------------------------------------------------------------------
// CIE 1931 2° standard observer colour matching functions, 360–830 nm, 1 nm
// Copied verbatim from pbrt-v4 src/pbrt/util/spectrum.cpp (nCIESamples = 471)
// ---------------------------------------------------------------------------
static constexpr int NrCIESamples = 471;

static constexpr float NrCIE_lambda[NrCIESamples] = {
...
static constexpr float NrCIE_X[NrCIESamples] = {
...
static constexpr float NrCIE_Y[NrCIESamples] = {
...
static constexpr float NrCIE_Z[NrCIESamples] = {
