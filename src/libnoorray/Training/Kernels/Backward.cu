...
NR_GPU_KERNEL void shadeGaussianTrainBackwardKernel(const GaussianTrainingKernelParams params)
{
    ...
    const glm::vec3 dLdColor = params.train.dLdImage[hit.sampleIndex]
        / static_cast<float>(params.train.samplesPerPixel);
    const glm::vec3 rayOrigin = hit.gaussianData;
    const glm::vec3 rayDirection = hit.direction;
