 ...
 * operation, matching Cycles' procedural-texture node layout.
 */
#pragma once
...
NR_GPU inline float svmWorleyDistance3d(const glm::vec3 p, const int x, const int y,
    const int z, const int xoff, const int yoff, const int zoff, const float jitter)
{
    const glm::vec3 diff = svmWorleyCellPosition3d(x, y, z, xoff, yoff, zoff, jitter) - p;
    ...
    const int style, const bool vectorResult)
{
    ...
    const glm::vec3 local = p - glm::floor(p);
    glm::vec3 nearest(1.0e6f);
    glm::vec3 minPos(0.0f);
    for (int x = -1; x <= 1; ++x)
        for (int y = -1; y <= 1; ++y)
            for (int z = -1; z <= 1; ++z) {
                const float distance = svmWorleyDistance3d(local, x, y, z, X, Y, Z, jitter);
    ...
    const int style, const bool vectorResult)
{
    const int X = static_cast<int>(floorf(p.x));
    const int Y = static_cast<int>(floorf(p.y));
    const glm::vec2 local = p - glm::floor(p);
    glm::vec3 nearest(1.0e6f);
