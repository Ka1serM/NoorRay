...
{
inline std::uint32_t fractalRotl32(const std::uint32_t x, const unsigned int k)
{
    return (x << k) | (x >> (32 - k));
}
inline std::uint32_t fractalBjfinal(std::uint32_t a, std::uint32_t b, std::uint32_t c)
{
...
inline std::uint32_t fractalHashInt3(const int x, const int y, const int z)
{
...
inline std::uint32_t fractalHashInt2(const int x, const int y)
{
    constexpr std::uint32_t seed = 0xdeadbeefu + (2u << 2u) + 13u;
    return fractalBjfinal(seed + static_cast<std::uint32_t>(x),
        seed + static_cast<std::uint32_t>(y), seed);
}
inline float fractalFade(const float t)
{
    return t * t * t * (t * (t * 6.0f - 15.0f) + 10.0f);
}
inline float fractalGradient(const std::uint32_t hash, const float x, const float y,
    const float z)
{
...
inline float svmPerlin3d(const glm::vec3 p)
{
...
inline float fractalGradient2d(const std::uint32_t hash, const float x,
    const float y)
{
...
inline float svmPerlin2d(const glm::vec2 p)
{
...
inline glm::vec3 svmPerlin3dVec3(const glm::vec3 p)
{
    const float floorX = floorf(p.x), floorY = floorf(p.y), floorZ = floorf(p.z);
    const int x = static_cast<int>(floorX), y = static_cast<int>(floorY);
    const int z = static_cast<int>(floorZ);
...
inline glm::vec3 svmPerlin2dVec3(const glm::vec2 p)
{
    const float floorX = floorf(p.x), floorY = floorf(p.y);
    const int x = static_cast<int>(floorX);
    const int y = static_cast<int>(floorY);
...
inline float svmFractal3d(glm::vec3 p, const int octaves, const float lacunarity,
...
inline glm::vec3 svmFractal3dVec3(glm::vec3 p, const int octaves,
...
inline float svmFractal2d(glm::vec2 p, const int octaves,
    const float lacunarity, const float diminish)
{
    float result = 0.0f, amplitude = 1.0f;
    for (int i = 0; i < octaves; ++i) {
...
inline glm::vec3 svmFractal2dVec3(glm::vec2 p, const int octaves,
    const float lacunarity, const float diminish)
{
    glm::vec3 result(0.0f);
    float amplitude = 1.0f;
