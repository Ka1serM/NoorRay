 ...
 *
 * MaterialX-only value node.  The structure follows the flattened Cycles SVM
 * node headers: all math lives beside its opcode, while SvmEval.h only loads
 * operands and stores results.  Formula is mx_artistic_ior.glsl from the
 * vendored MaterialX pbrlib standard library.
 */
