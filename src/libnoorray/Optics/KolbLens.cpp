...
ParsedLens loadZmx(const std::string& lensPath, const std::vector<std::string>& catalogPaths) {
    ...
    struct Raw { float curvature{}, thickness{}, semiDiameter{}; float conic{}; std::array<float, nr::graphics::MaxAsphereTerms> asphere{}; std::string glass; bool stop{}; };
    ...
    while (std::getline(in, line)) {
        ...
        if (command == "CURV" && f.size() >= 2) current->curvature = number(f[1], lensPath, lineNumber);
            ...
            if (term >= 4 && term < 4 + static_cast<int>(nr::graphics::MaxAsphereTerms))
        ...
        else if (command == "PARM" && f.size() >= 3) { const int i = std::stoi(f[1]); if (i == 1) current->conic = number(f[2], lensPath, lineNumber); else if (i >= 2 && i < 2 + static_cast<int>(nr::graphics::MaxAsphereTerms)) current->asphere[i - 2] = number(f[2], lensPath, lineNumber); else throw std::runtime_error(lensPath + ":" + std::to_string(lineNumber) + ": unsupported asphere term"); }
    ...
    ParsedLens result; nr::graphics::Lens& lens = result.snapshot;
    const auto setMedium = [](nr::graphics::Medium& target, const SellmeierCoefficients& source) {
        target.b = source.b;
        target.c = source.c;
    };
    setMedium(lens.media[0], constantIorSellmeier(1.f));
    ...
    for (size_t backwards = raw.size() - 1; backwards > 0; --backwards) {
        if (lens.surfaceCount == nr::graphics::MaxLensSurfaces) throw std::runtime_error(lensPath + ": too many optical surfaces");
        const Raw& r = raw[backwards]; nr::graphics::Surface& s = lens.surfaces[lens.surfaceCount++];
        s.z = imageZ - z[backwards]; s.radius = fabsf(r.curvature) > 1e-8f ? -1.f / r.curvature : 0.f;
        s.apertureRadius = std::max(0.001f, r.semiDiameter); s.conic = r.conic;
        std::copy(r.asphere.begin(), r.asphere.end(), s.asphere);
        s.isStop = r.stop;
        if (!r.glass.empty() && r.glass != "AIR") { const auto it = glasses.find(r.glass); if (it == glasses.end()) throw std::runtime_error(lensPath + ": glass '" + r.glass + "' is absent from the supplied AGF catalogs"); setMedium(lens.media[++medium], it->second.coefficients); }
        else { setMedium(lens.media[++medium], constantIorSellmeier(1.f)); }
    ...
    for (uint32_t i = 0; i < lens.surfaceCount; ++i) {
        const nr::graphics::Surface& surface = lens.surfaces[i];
        const uint32_t outgoingMedium = std::min(surface.mediumAfter, lens.mediumCount - 1);
        const SellmeierCoefficients incoming{
            lens.media[incomingMedium].b, lens.media[incomingMedium].c};
        const SellmeierCoefficients outgoing{
            lens.media[outgoingMedium].b, lens.media[outgoingMedium].c};
        const float nIncoming = sellmeierIor(incoming, FraunhoferGreenNm);
        const float nOutgoing = sellmeierIor(outgoing, FraunhoferGreenNm);
