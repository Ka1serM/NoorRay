...
: context(context), scene(scene), name(objFilePath) {
    ...
    blas = std::make_unique<Accel>(context, geometry, static_cast<uint32_t>(indices.size() / 3),
                                   vk::AccelerationStructureTypeKHR::eBottomLevel);
}

uint64_t MeshAsset::getBlasAddress() const {
    return blas.get()->buffer.getDeviceAddress();
}
...
void MeshAsset::setBufferIndex(uint32_t newIndex) {
    index = newIndex;
}
