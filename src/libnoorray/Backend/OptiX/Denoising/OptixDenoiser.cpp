...
nr::OptixDenoiser::~OptixDenoiser() noexcept
{
    reset();
}

void nr::OptixDenoiser::ensure(const OptixDeviceContext context,
...
const void* nr::OptixDenoiser::run(const OptixDeviceContext context,
    const cudaStream_t stream, const void* input, const void* albedoGuide,
    const void* normalGuide, void* externalOutput,
    const uint32_t width, const uint32_t height)
{
    ...
    if (externalOutput == nullptr) {
        const size_t outputBytes = rowStride * height;
        if (!output || output.size() != outputBytes)
            output.allocate(outputBytes, stream);
        externalOutput = output.get();
    }
    ...
    layer.output = {reinterpret_cast<CUdeviceptr>(externalOutput), width, height,
        static_cast<unsigned int>(rowStride), sizeof(glm::vec4), OPTIX_PIXEL_FORMAT_FLOAT4};
...
void nr::OptixDenoiser::reset() noexcept
{
    output.reset();
    intensity.reset();
    state.reset();
