...
// Compact non-scattering results produced while an OSL closure tree is walked.
// Scattering lobes bypass this structure and are built directly in the spectral
// composite BSDF, avoiding a second per-hit lobe array and conversion pass.

// Optional outputs. A material program only writes what its graph actually
// produces, so consumers must not read a field whose flag is clear.
...
// Compact non-scattering outputs. Spectral lobes are constructed directly in
// NoorRayCompositeBsdf while the OSL closure tree is walked.
...
// The geometric state a material program is evaluated against.
//
// This is the input half of the same ABI. It is passed by pointer, so a graph
// that reads only UVs costs the same call as one that reads everything; the
// caller decides which fields are worth computing from the program's feature
// flags.
